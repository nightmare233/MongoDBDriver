...
namespace MongoDB.Driver.Tests
{
    public class AsyncCursorHelperTests
    {
        ...
        private Task<IAsyncCursor<int>> SetupResultInFirstBatchAsync()
        {
        ...
        private Task<IAsyncCursor<int>> SetupResultInSecondBatchAsync()
        {
            var mockCursor = new Mock<IAsyncCursor<int>>();
        ...
        private Task<IAsyncCursor<int>> SetupNoResultIn1BatchAsync()
        {
            var mockCursor = new Mock<IAsyncCursor<int>>();
        ...
        private Task<IAsyncCursor<int>> SetupNoResultInTwoBatchesAsync()
        {
            var mockCursor = new Mock<IAsyncCursor<int>>();
            mockCursor.SetupSequence(c => c.MoveNextAsync(It.IsAny<CancellationToken>())).Returns(Task.FromResult(true)).Returns(Task.FromResult(false));
